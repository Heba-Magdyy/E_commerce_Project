...
const AddProductForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Manually assign a unique ID to the new product
      const newProduct = { ...product, id: generateUniqueId() };
      
      // Post the product to the server (if your server supports ID assignment, remove the id property here)
      await axios.post('https://dummyjson.com/products/add', newProduct);
      
      // Store the product in localStorage
      const storedProducts = JSON.parse(localStorage.getItem('products')) || [];
      storedProducts.push(newProduct); // Add the product with the new unique ID
